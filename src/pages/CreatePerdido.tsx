...
const CreatePerdidoPage = () => {
  ...
  const handlePublish = async () => {
    if (!user || !perdidoCategoriaId) return;
    ...
    try {
      const { data: anuncio, error } = await supabase
        ...
        .insert({
          ...
          gratuito: true,
          categoria_id: perdidoCategoriaId,
          ...
          ultima_vez_visto: form.ultima_vez_visto.trim() || null,
        })
        .select()
        .single();
    ...
    <div className="pb-20 md:pb-8 animate-fade-in">
      <div className="page-container py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <h1 className="font-display text-lg font-bold">Perdido / Encontrado</h1>
        ...
        <div className="space-y-4">
          {step === 0 && (
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-4">Adicione fotos para ajudar na identificação</p>
          ...
          {step === 1 && (
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>O que perdeu / encontrou? *</Label>
                <Input placeholder="Ex: Cachorro caramelo com coleira azul" maxLength={150} value={form.titulo} onChange={(e) => update('titulo', e.target.value)} />
                <p className="text-xs text-muted-foreground text-right">{form.titulo.length}/150</p>
              </div>
              <div className="space-y-2">
                <Label>Descrição detalhada *</Label>
                <Textarea placeholder="Descreva com detalhes para facilitar a identificação..." maxLength={5000} rows={5} value={form.descricao} onChange={(e) => update('descricao', e.target.value)} />
                <p className="text-xs text-muted-foreground text-right">{form.descricao.length}/5000</p>
              </div>
              <div className="space-y-2">
                <Label>Última vez visto</Label>
                <Input placeholder="Ex: Ontem às 18h próximo à padaria" value={form.ultima_vez_visto} onChange={(e) => update('ultima_vez_visto', e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Recompensa (opcional)</Label>
                <Input placeholder="Ex: R$ 100 ou Gratificação" value={form.recompensa} onChange={(e) => update('recompensa', e.target.value)} />
              </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Localização onde foi visto</Label>
                <Input placeholder="Ex: Próximo à escola municipal" value={form.localizacao_texto} onChange={(e) => update('localizacao_texto', e.target.value)} />
              </div>
            </div>
          )}
          ...
          {step === 4 && (
            <div className="space-y-4">
              <h2 className="font-display text-base font-semibold">Revisão</h2>
              <div className="rounded-lg border bg-card p-4 space-y-3 text-sm">
                <div><span className="text-muted-foreground">Título:</span> <strong>{form.titulo || '—'}</strong></div>
                {form.ultima_vez_visto && <div><span className="text-muted-foreground">Última vez visto:</span> {form.ultima_vez_visto}</div>}
                {form.recompensa && <div><span className="text-muted-foreground">Recompensa:</span> {form.recompensa}</div>}
...
export default CreatePerdidoPage;
